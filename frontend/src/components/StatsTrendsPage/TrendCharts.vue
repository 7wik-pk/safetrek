...
async function load() {
  ...
  try {
    ...
    error.value = e?.response?.data?.detail || e?.response?.data || e.message || 'Request failed'
    monthly.value = null
    yearly.value = null
  } finally {
    loading.value = false
...
//  Chart data
function mkSeries(label: string, data: number[], stroke: string, fill: string) {
  return {
    label,
    data,
    borderColor: stroke,
  ...
  if (monthly.value) {
    ...
    return {
      ...
      datasets: [
        mkSeries(
          'Crashes',
          monthly.value.map((d) => d.crashes),
          THEME.crashes,
          THEME.crashesFill,
        ),
        mkSeries(
          'Total injuries',
          monthly.value.map((d) => d.total_injuries),
          THEME.injuries,
          THEME.injuriesFill,
        ),
        mkSeries(
          'Serious injuries',
          monthly.value.map((d) => d.serious_injuries),
          THEME.serious,
          THEME.seriousFill,
        ),
      ],
  ...
  if (yearly.value) {
    ...
    return {
      ...
      datasets: [
        mkSeries(
          'Crashes',
          yearly.value.map((d) => d.crashes),
          THEME.crashes,
          THEME.crashesFill,
        ),
        mkSeries(
          'Total injuries',
          yearly.value.map((d) => d.total_injuries),
          THEME.injuries,
          THEME.injuriesFill,
        ),
        mkSeries(
          'Serious injuries',
          yearly.value.map((d) => d.serious_injuries),
          THEME.serious,
          THEME.seriousFill,
        ),
      ],
